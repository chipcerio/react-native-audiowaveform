...
public class CheapWAV extends CheapSoundFile {
    ...
    public void WriteFile(File outputFile, int startFrame, int numFrames)
        ...
        int additionalFramesAtEnd = 0;
        for (int i = 0; i < numFrames; i++) {
            int targetIndex = startFrame + i;
            if (targetIndex >= mFrameLens.length)
            {
                additionalFramesAtEnd = (startFrame + numFrames) - mFrameLens.length;
                numFrames -= additionalFramesAtEnd;
                break;
            } else {
        ...
        if (additionalFramesAtEnd > 0)
        {
            byte[] additionalBuffer = new byte[mFrameBytes * additionalFramesAtEnd];
            out.write(additionalBuffer, 0, additionalBuffer.length);
        }
